...
void LeedsGLRenderWidget::paintGL()
{
    ...
    glDrawPixels(int(leedsGL.frameBuffer.width), int(leedsGL.frameBuffer.height), GL_RGBA,
                 GL_UNSIGNED_BYTE, leedsGL.frameBuffer.block);
...
void LeedsGLRenderWidget::paintLeedsGL()
    ...
    std::cout << "width " << float(leedsGL.frameBuffer.width) << "height " << float(leedsGL.frameBuffer.height)<< std::endl;
    if(renderParameters->perspective){
        leedsGL.enable(LeedsGL::PERSPECTIVE);
        if (aspectRatio > 1.0f)
            P = LeedsGLUtils::calculateProjectionFrustum(-0.01f*aspectRatio,
                                                         0.01f*aspectRatio,
                                                         -0.01f,
                                                         0.01f,
                                                         0.01f,
